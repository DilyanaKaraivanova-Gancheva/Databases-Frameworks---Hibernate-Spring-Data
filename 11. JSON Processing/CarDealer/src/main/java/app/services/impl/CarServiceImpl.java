...
public class CarServiceImpl implements CarService{
    ...
    @Override
    public List<CarsPartsView> getAllCarsWithTheirParts() {
        List<Car> cars = this.carRepository.findAll();
        List<CarsPartsView> carPartsViews = new ArrayList<>();
        for (Car car : cars) {
            CarsPartsView carPartsView = new CarsPartsView();
            CarView carView = this.mapper.map(car, CarView.class);
            carPartsView.setCar(carView);
            for (Part part : car.getParts()) {
                PartView partView = this.mapper.map(part, PartView.class);
                carPartsView.getParts().add(partView);
            }
            carPartsViews.add(carPartsView);
        }
